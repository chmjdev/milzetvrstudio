...
public static class NativePreview {
 ...
 static void VerifyRunning() {
  try {
   ...
   UnityEngine.Object.DestroyImmediate(go);ReferenceVerification.Run(Path.GetDirectoryName(path),output);ProjectionVerification.Run(Path.GetDirectoryName(path),output);Debug.Log("MILZET_NATIVE_VISUAL_PASS");VideoVerification.StartCheck(Path.GetDirectoryName(path),output);
