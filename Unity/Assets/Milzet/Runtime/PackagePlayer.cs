...
public sealed class PackagePlayer : MonoBehaviour {
 ...
 GameObject content;string sessionId;int sequence;string packagePath="";Vector2 scroll;
 ...
 public void Open(string text) {
  ...
  Clear();Loaded=next;PlateTexture=texture;owned.Add(texture);foreach(var clip in clips){Clips.Add(clip.Key,clip.Value);owned.Add(clip.Value);}Events.Clear();sequence=0;sessionId=Guid.NewGuid().ToString("N");SimulatedHostRelease=false;SelectedId="";
  ...
  ViewCamera=new GameObject("Package camera").AddComponent<Camera>();ViewCamera.transform.SetParent(content.transform,false);ViewCamera.transform.position=new Vector3(0,1.5f,-1);ViewCamera.transform.rotation=Quaternion.identity;ViewCamera.fieldOfView=60;ViewCamera.nearClipPlane=.1f;ViewCamera.farClipPlane=100;ViewCamera.clearFlags=CameraClearFlags.SolidColor;ViewCamera.backgroundColor=new Color(21/255f,42/255f,39/255f);ViewCamera.gameObject.AddComponent<AudioListener>();
