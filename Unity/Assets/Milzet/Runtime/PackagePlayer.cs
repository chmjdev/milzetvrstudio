...
public sealed partial class PackagePlayer : MonoBehaviour {
 ...
 public void Open(string text) {
  PackageReader.Require(!VideoLoading || preparedVideo!=null,"Video import already in progress.");
  var next=PackageReader.Open(text);if(next.Manifest.assets.First(a=>a.id==next.Manifest.plate.assetId).mime=="video/mp4" && preparedVideo==null){BeginVideo(text,next);return;}var texture=new Texture2D(2,2,TextureFormat.RGBA32,false);var clips=new Dictionary<string,AudioClip>();
  ...
  if(preparedVideo!=null){Video=preparedVideo;Video.transform.SetParent(content.transform,false);owned.Add(Video.targetTexture);preparedVideo=null;}Physics.SyncTransforms();Message="Package loaded. Desktop visual preview; no headset acceptance.";
 }
 ...
 public void Clear() {if(Video!=null)Video.Stop();Video=null;if(Narration!=null)Narration.Stop();if(content!=null)DestroyImmediate(content);foreach(var item in owned)if(item!=null)DestroyImmediate(item);owned.Clear();Markers.Clear();Clips.Clear();Loaded=null;Session=null;}
