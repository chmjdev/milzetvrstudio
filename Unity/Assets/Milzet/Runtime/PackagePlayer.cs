...
[Serializable] public class HostEvent {
 public string activityId="";public string response="";public string eventId;public string packageId;public string revision;public string phase;public string type;public string hotspotId;
}
public sealed partial class PackagePlayer : MonoBehaviour {
 ...
 GameObject content;string sessionId;int sequence;string packagePath="";Vector2 scroll;float yaw,pitch;string activityResponse="";readonly HashSet<string> pausedCues=new HashSet<string>();
 ...
 public void Open(string text) {
  PackageReader.Require(!ModelsLoading,"Model import already in progress.");PackageReader.Require(!VideoLoading || preparedVideo!=null,"Video import already in progress.");
  var next=PackageReader.Open(text);if(next.Manifest.objects!=null && next.Manifest.objects.Length>0 && preparedModels==null){BeginModels(text,next);return;}if(next.Manifest.assets.First(a=>a.id==next.Manifest.plate.assetId).mime=="video/mp4" && preparedVideo==null){BeginVideo(text,next);return;}var texture=new Texture2D(2,2,TextureFormat.RGBA32,false);var clips=new Dictionary<string,AudioClip>();
  try {if(preparedVideo==null)PackageReader.Require(ImageConversion.LoadImage(texture,next.Assets[next.Manifest.plate.assetId]),"Unable to decode image plate.");foreach(var asset in next.Manifest.assets.Where(a=>a.mime=="audio/wav"))clips.Add(asset.id,WaveDecoder.Decode(next.Assets[asset.id],asset.id));}
  catch{DestroyImmediate(texture);foreach(var clip in clips.Values)DestroyImmediate(clip);throw;}
  Clear();Loaded=next;PlateTexture=texture;owned.Add(texture);foreach(var clip in clips){Clips.Add(clip.Key,clip.Value);owned.Add(clip.Value);}Events.Clear();pausedCues.Clear();sequence=0;sessionId=Guid.NewGuid().ToString("N");SimulatedHostRelease=false;SelectedId="";yaw=0;pitch=0;
  Session=new PackageSession(next,(type,phase,id)=>Record(type,phase,id));Session.ActivityEvent=(type,id,response)=>Record(type,Session.PhaseId,"",id,response);
  ...
  if(preparedVideo!=null){Video=preparedVideo;Video.transform.SetParent(content.transform,false);owned.Add(Video.targetTexture);preparedVideo=null;}AdoptModels();Physics.SyncTransforms();Message="Package loaded. Desktop visual preview; no headset acceptance.";
 }
 void Record(string type,string phase,string hotspotId="",string activityId="",string response=""){var e=new HostEvent{eventId=sessionId+":"+(++sequence),packageId=Loaded.Manifest.id,revision=Loaded.Revision,phase=phase,type=type,hotspotId=hotspotId,activityId=activityId,response=response};Events.Add(e);HostEventEmitted?.Invoke(e);}
 ...
 public bool SelectRay(Ray ray) {if(Physics.Raycast(ray,out var hit,100)){var marker=Markers.FirstOrDefault(p=>p.Value==hit.collider.gameObject);if(marker.Value!=null){Select(marker.Key);return true;}if(modelTargets.TryGetValue(hit.collider.gameObject,out var id)){Select(id);return true;}}return false;}
 ...
 void Update(){if(Video==null || Loaded==null || Session==null)return;var cue=(Loaded.Manifest.activities??new Activity[0]).FirstOrDefault(a=>a.phase==Session.PhaseId && !Session.CompletedActivities.Contains(a.id));if(cue!=null && cue.startTime>0 && Video.time>=cue.startTime && pausedCues.Add(cue.id))Video.Pause();}
 void OnGUI() {
 ...
 GUILayout.BeginArea(new Rect(12,12,width,Screen.height-24),GUI.skin.box);scroll=GUILayout.BeginScrollView(scroll);GUILayout.Label("MILZET / NATIVE PACKAGE PREVIEW");packagePath=GUILayout.TextField(packagePath);if(GUILayout.Button("Open local package"))Try(()=>OpenFile(packagePath));
  if(Loaded!=null){GUILayout.Label(Loaded.Manifest.title);if(Loaded.Manifest.context!=null){GUILayout.Label(Loaded.Manifest.context.occupationRef);GUILayout.Label(Loaded.Manifest.context.moduleRefs);GUILayout.Label(Loaded.Manifest.context.sourceCitation);}if(Video!=null){if(GUILayout.Button(Video.isPlaying?"Pause clip":"Play clip")){if(Video.isPlaying)Video.Pause();else Video.Play();}float seek=GUILayout.HorizontalSlider((float)Video.time,0,(float)Video.length);if(Mathf.Abs(seek-(float)Video.time)>.1f)Video.time=seek;}if(Loaded.Manifest.plate.projection!="flat"){GUILayout.Label("Look direction");float heading=GUILayout.HorizontalSlider(yaw,-180,180);float elevation=GUILayout.HorizontalSlider(pitch,-89,89);SetView(heading,elevation);}if(Loaded.Manifest.fixture)GUILayout.Label("TEST FIXTURE — NOT CLIENT INSTRUCTION");GUILayout.Label("Phase: "+Session.PhaseId);foreach(var id in Loaded.Manifest.phases.First(p=>p.id==Session.PhaseId).hotspotIds){if(GUILayout.Button(Loaded.Manifest.hotspots.First(h=>h.id==id).label))Try(()=>Select(id));}var activity=(Loaded.Manifest.activities??new Activity[0]).FirstOrDefault(a=>a.phase==Session.PhaseId && !Session.CompletedActivities.Contains(a.id));if(activity!=null){GUILayout.Label(activity.prompt);GUILayout.Label(activity.citation);if(activity.kind=="quiz"){foreach(var choice in activity.choices)if(GUILayout.Button(choice))Try(()=>Session.Respond(activity.id,choice,Video!=null?Video.time:0));}else{if(activity.kind!="acknowledgement" && activity.kind!="step")activityResponse=GUILayout.TextField(activityResponse);if(GUILayout.Button("Submit activity response"))Try(()=>{Session.Respond(activity.id,activity.kind=="acknowledgement" || activity.kind=="step"?"acknowledged":activityResponse,Video!=null?Video.time:0);activityResponse="";});}if(activity.hint!="" && activity.phase!="prove" && GUILayout.Button("Show activity hint"))Try(()=>Message=Session.Hint(activity.id));}SimulatedHostRelease=GUILayout.Toggle(SimulatedHostRelease,"Simulate host release (test host)");GUI.enabled=!Session.Complete;if(GUILayout.Button("Continue phase"))Try(Advance);GUI.enabled=true;if(GUILayout.Button("Play package audio"))Try(PlaySelectedAudio);GUILayout.Label("Host events: "+Events.Count);}
  GUILayout.Label(Message);GUILayout.EndScrollView();GUILayout.EndArea();
 }
 public void Clear() {ClearModels();if(Video!=null)Video.Stop();Video=null;if(Narration!=null)Narration.Stop();if(content!=null)DestroyImmediate(content);foreach(var item in owned)if(item!=null)DestroyImmediate(item);owned.Clear();Markers.Clear();Clips.Clear();Loaded=null;Session=null;}
 void OnDestroy(){DiscardPreparedModels();Clear();}
}
}
