...
public sealed partial class PackagePlayer {
 ...
 IEnumerator PrepareVideo(string text,LoadedPackage next) {
  ...
  if(!player.isPrepared || failed!=null){Message="Video import failed: "+(failed??"preparation timed out");DestroyImmediate(temporary);DiscardPreparedModels();VideoLoading=false;yield break;}
  bool immersive=next.Manifest.plate.projection!="flat";int ratio=next.Manifest.plate.projection=="equirect360"?2:1;
  if(player.width==0 || player.height==0 || player.width>4096 || player.height>4096 || immersive && player.width!=player.height*ratio){Message="Unsupported clip dimensions or mono projection; maximum 4096 pixels per side.";DestroyImmediate(temporary);DiscardPreparedModels();VideoLoading=false;yield break;}
