...
public sealed partial class PackagePlayer {
 ...
 async void BeginModels(string text,LoadedPackage next){
  PackageReader.Require(!ModelsLoading,"Model import already in progress.");ModelsLoading=true;Message="Opening 3D content…";var temporary=new GameObject("Preparing models");temporary.transform.SetParent(transform,false);temporary.SetActive(false);var imports=new List<GltfImport>();var objects=new Dictionary<string,GameObject>();
  try {
   ...
   foreach(var o in next.Manifest.objects){var group=objects[o.id];if(o.parentId!="")group.transform.SetParent(objects[o.parentId].transform,false);var imported=new GltfImport();imports.Add(imported);Debug.Log("MILZET_MODEL_LOAD "+o.assetId);PackageReader.Require(await imported.Load(next.Assets[o.assetId]),"GLB decode failed.");Debug.Log("MILZET_MODEL_DECODED "+o.assetId);var meshRoot=new GameObject("glTF orientation");meshRoot.transform.SetParent(group.transform,false);meshRoot.transform.localRotation=Quaternion.Euler(0,180,0);PackageReader.Require(await imported.InstantiateMainSceneAsync(meshRoot.transform),"GLB instantiation failed.");Debug.Log("MILZET_MODEL_INSTANTIATED "+o.assetId);}
   if(this==null){foreach(var imported in imports)imported.Dispose();return;}
   preparedImports=imports;preparedObjects=objects;preparedModels=temporary;ModelsLoading=false;OpenScene(text);
  }catch(Exception e){Message="Model import failed: "+e.Message;Debug.LogException(e);foreach(var imported in imports)imported.Dispose();if(temporary!=null)DestroyImmediate(temporary);preparedModels=null;preparedImports=null;preparedObjects=null;ModelsLoading=false;}
 }
 void AdoptModels(){
