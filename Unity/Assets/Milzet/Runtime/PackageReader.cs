...
[DataContract] public class Manifest { [DataMember(EmitDefaultValue=false)] public SceneObject[] objects; [DataMember(IsRequired=true)] public string id; [DataMember(IsRequired=true)] public string title; [DataMember(IsRequired=true)] public bool fixture; [DataMember(IsRequired=true)] public string entryPhase; [DataMember(IsRequired=true)] public Asset[] assets; [DataMember(IsRequired=true)] public Plate plate; [DataMember(IsRequired=true)] public Hotspot[] hotspots; [DataMember(IsRequired=true)] public Phase[] phases; }
...
public static class PackageReader {
 ...
 static void Shape(string envelope,string manifest) {
  ...
  var m=Tree(manifest);Fields(m,"id,title,fixture,entryPhase,assets,plate,hotspots,phases"+(m.Element("objects")==null?"":",objects"));foreach(var o in m.Element("objects")?.Elements()??Enumerable.Empty<XElement>())Fields(o,"id,label,assetId,parentId,position,rotation,scale,visible,hotspotId");Fields(m.Element("plate"),"assetId,projection");
 ...
 static void Validate(Manifest m) {
  ...
  var visited=new HashSet<string>();var current=m.entryPhase;while(current!=""){Require(visited.Add(current),"Cyclic phases.");current=m.phases.First(p=>p.id==current).next;}Require(visited.Count==phases.Count,"Unreachable phase.");ModelContract.Validate(m);
 }
}
