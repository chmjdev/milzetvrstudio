...
public sealed class CommonsVerification : MonoBehaviour {
 ...
 IEnumerator Check(PackagePlayer player,string directory){
  int count=0,activities=0,immersive=0;bool videoAdvanced=false;
  foreach(var filename in new[]{"07-Trench-observation.json","08-Toolbox-talk.json","09-Daily-site-log.json","10-Assessment-observation.json","11-First-aid-familiarisation.json","12-Site-Entrance.json","13-Site-Interior.json","14-Control-Centre.json","15-Turbine-Hall.json","16-Seminar-Room.json","17-Safety-Lab.json","18-Assembly-Point.json"}){
   player.OpenFile(Path.Combine(directory,filename));float end=Time.realtimeSinceStartup+40;while((player.ModelsLoading || player.VideoLoading) && Time.realtimeSinceStartup<end)yield return null;PackageReader.Require(player.Loaded.Manifest.id.StartsWith("commons-"),"Example import failed.");player.ShowWorldControls(true);PackageReader.Require(player.Loaded.Manifest.provenance.Length==1,"Example attribution missing.");if(player.Loaded.Manifest.plate.projection!="flat")immersive++;
  ...
  File.WriteAllText(Path.Combine(directory,"native-verification.json"),"{\"passed\":true,\"actualStandalone\":true,\"packages\":"+count+",\"immersive\":"+immersive+",\"activities\":"+activities+",\"videoAdvanced\":"+videoAdvanced.ToString().ToLowerInvariant()+",\"physicalHeadset\":false}");Debug.Log("MILZET_COMMONS_PASS");Application.Quit(0);
 }
}
}
