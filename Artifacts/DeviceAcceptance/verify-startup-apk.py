...
assert "name='com.binteca.interactive.milzet.viewer' versionCode='5' versionName='1.1.0'" in bad
...
receipt=dict(passed=True,versionName='1.1.0',versionCode=5,applicationId='com.binteca.interactive.milzet.viewer',bytes=apk.stat().st_size,sha256=hashlib.sha256(apk.read_bytes()).hexdigest(),signatureV2Verified=True,handAndScenePermissions=True,libraries=libs,bootConfig=boot,signature=sig,physicalHeadset=False)
(root/'Artifacts/DeviceAcceptance/startup-artifact.json').write_text(json.dumps(receipt,indent=2)+'\n')
(root/'Artifacts/DeviceAcceptance/startup-manifest.txt').write_text(manifest)
