...
export function Scene({loaded,onSelect}) {
 ...
 useEffect(()=>{
  ...
  try {
   ...
   const scene=new T.Scene();scene.background=new T.Color('#152a27');const camera=new T.PerspectiveCamera(60,1,.1,100);camera.position.set(0,1.5,1);
   ...
   const geometry=new T.PlaneGeometry(4,2.4);const material=new T.MeshBasicMaterial({map:texture});resources.push(geometry,material);
   const plate=new T.Mesh(geometry,material);plate.position.set(0,1.5,-2);scene.add(plate);
   const targets=loaded.manifest.hotspots.map(h=>{const g=new T.SphereGeometry(.065,16,12),m=new T.MeshBasicMaterial({color:'#d6f895'});resources.push(g,m);const mesh=new T.Mesh(g,m);mesh.position.set((h.x-.5)*4,1.5+(.5-h.y)*2.4,-1.93);mesh.userData.id=h.id;scene.add(mesh);return mesh;});
   const ray=new T.Raycaster();
   const click=e=>{const b=renderer.domElement.getBoundingClientRect();ray.setFromCamera(new T.Vector2((e.clientX-b.left)/b.width*2-1,-(e.clientY-b.top)/b.height*2+1),camera);const hit=ray.intersectObjects(targets)[0];if(hit)select.current(hit.object.userData.id);};
